...
[Cursor](https://cursor.com) 是一款基于 VS Code 的 AI 驱动 IDE。它内置 AI 编程助手，支持通过自定义 Base URL 接入 New API。
...
3. 输入你的 New API 令牌：`sk-你的NewAPI令牌`
...
1. 点击 **+ Add Model**
2. 输入你想使用的模型名称（必须与 New API 中配置的模型名一致）
...
如果遇到模型冲突，先取消勾选 Cursor 原有的模型（如官方 GPT-4o），只保留你通过 New API 添加的模型。
...
2. 如果 Agent 模式仍有问题，可能需要等待 New API 更新以支持该格式
