...
[Cline](https://cline.bot) 是一款强大的 VS Code AI 编程扩展（原名 Claude Dev）。它支持多种 API Provider，可以轻松接入 New API。
...
| **API Key** | `sk-你的NewAPI令牌` | New API 令牌 |
...
::: tip
模型名称必须与你在 New API 后台配置的模型名 **完全一致**。
:::
...
| 模型不可用 | 模型名称不匹配 | 在 New API 后台确认模型名 |
