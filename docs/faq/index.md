...
### 我可以通过 New API 使用哪些模型？

取决于你的 New API 管理员配置了哪些渠道。常见可用模型：
...
具体可用模型请咨询你的 New API 管理员。

### 模型名称在 New API 后台和工具中不一致怎么办？

你在工具中填写的模型名称必须与 **New API 后台配置的模型名** 完全一致。如有疑问，在 New API 后台的「渠道」或「模型」页面确认实际的模型 ID。

## 网络相关

### 我在公司内网，需要代理才能访问 New API，怎么配置？
...
1. 检查 New API 和你之间的网络延迟
2. 在 New API 后台查看渠道的响应时间
