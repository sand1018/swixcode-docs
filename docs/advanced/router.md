...
CCR 是一个本地代理服务，作为 Claude Code 和 New API 之间的 **中间层**：

```
Claude Code  →  CCR (本地:3456)  →  New API  →  AI 服务商
```
...
| `Providers[].api_base_url` | New API 地址（需要 `/v1`） |
| `Providers[].api_key` | New API 令牌 |
